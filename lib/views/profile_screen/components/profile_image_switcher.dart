...
class ProfileImageSwitcher extends StatefulWidget {
  ...
  const ProfileImageSwitcher({super.key, required this.viewModel});
  ...
  void initState() {
    ...
    Future.delayed(const Duration(milliseconds: 10), () {
      if (mounted) {
        setState(() {
          _currentIndex =
              (_currentIndex + 1) % widget.viewModel.imagesList.length;
        });
    ...
    return Stack(
      children: [
        FadeTransition(
          ...
          child: widget.viewModel.imagesList[_currentIndex],
        ),
        FadeTransition(
